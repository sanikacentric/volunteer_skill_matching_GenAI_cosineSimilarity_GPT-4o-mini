...
User fills form → React sends POST /volunteers with JSON
FastAPI validates input via VolunteerProfile Pydantic model
volunteer_to_text() converts structured fields into one natural-language sentence
get_embedding() calls OpenAI and receives a 1536-float vector
Profile dict + embedding + UUID + timestamp appended to volunteers_db
React re-fetches /volunteers and updates the sidebar list
Reporting a crisis:

Identical flow using crisis_to_text() → get_embedding() → crises_db
Running a match:

User selects a crisis → React sends POST /match with {crisis_id, top_k: 5}
Backend retrieves pre-computed crisis embedding from crises_db
Loops all volunteers: cosine_similarity(crisis_embedding, volunteer_embedding)
Sorts descending, takes top-K
Calls explain_match() once per top volunteer (GPT-4o-mini)
Returns full ranked list: scores, percentages, skill tags, AI explanations
React renders animated match cards with score bars and medal ranks
...
Wipes both volunteers_db and crises_db (resets to empty lists)
Instantiates 5 VolunteerProfile objects and 1 CrisisReport object inline in Python
Calls register_volunteer() for each volunteer — this triggers real OpenAI embedding calls, generating actual 1536-dim vectors for each profile
Calls report_crisis() for the crisis — same real embedding generation
Returns {"message": "Demo data seeded!", "volunteers": 5, "crises": 1}
...
Maria Santos — nurse + Spanish speaker in Los Angeles (same city, exact skills match)
Carlos Rivera — paramedic + Spanish + trilingual (medical + language match)
Aisha Rahman — trauma support + counseling (needs "trauma support" explicitly)
James Okonkwo — logistics (lower score, no medical or Spanish)
Chen Wei — communications/tech (lowest score, no medical relevance)
...
Get your key at: https://platform.openai.com/api-keys Required models: text-embedding-3-small and gpt-4o-mini (both available on free tier with credits)
...
On the Dashboard, click "Load Demo Data" (top right, green button)
...
The stat cards update: 5 volunteers, 1 crisis
Navigate to the Match Engine tab
Select "[CRITICAL] Medical Emergency in Spanish-Speaking Community" from the dropdown
Click "Run AI Match"
This computes 5 cosine similarities + calls GPT-4o-mini 5 times for explanations
Wait ~5–10 seconds
View the ranked results with match scores, skill tags, and AI explanations
